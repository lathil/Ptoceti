...
export class WatchesService {
    ...
    constructor(http: HttpClient, storage: LocalStorage, searchService: SearchService,) {
        ...
        this.cache = new ReplaySubject<WatchService>(1);
    ...
    initialize(rootUrl: string, serviceUrl: string ) {
        ...
        this.storage.getItem(serviceUrl).subscribe((response) => {
            if (response) {
                let watchservice: WatchService = new WatchService;
                watchservice.parse(response);
                this.cache.next( watchservice );
            } else {
                // not available, get it from backend server
                this.http.get(serviceUrl).pipe(map(this.handleWatchServiceResponse)).subscribe(watchservice => {
    ...
    getWatchesList() {
        ...
        this.storage.getItem(WatchesService.watchesListKey).pipe(
            map(item => {
                if (item != null) {
            ...
            .subscribe((list ) => {
                if ( list != null ) {
                    ...
                    observableConcat<Watch>(...observables).subscribe((watch) => {
                        this.watchStream.next(new WatchAction(Action.Add, watch))
                    });
    ...
    getWatch(watchUrl ): Observable<Watch> {
        ...
        this.storage.getItem(watchUrl).pipe(
            map(item => {
                if (item != null) {
                    let watchItem: Watch = new Watch();
                    watchItem.parse(item);
                    return watchItem;
                }
            }))
            .subscribe((watchItem ) => {
    ...
    refreshWatch(watchUrl: string, observer: Observer<Watch> ) {
        this.http.get(watchUrl).pipe(map(response => {
            let watchItem: Watch = new Watch();
    ...
    createWatch() {
        ...
        this.getWatchService().subscribe(watchService => {
            ...
            this.http.post(this.serviceUrl, null).pipe(map(response => {
                let watchItem: Watch = new Watch();
                watchItem.parse(response);
                return watchItem;
            }))
        ...
        this.storage.getItem(watchUrl).pipe(
            ...
            .subscribe((watchItem ) => {
                if ( watchItem != null ) {
                    ...
                    this.http.post(deleteUrl, null).pipe(map(response => {
                        let nil: Nil = new Nil();
                        nil.parse(response);
                        return nil;
                    }))
    ...
    getWatchCount(watchUrl ): Observable<Number> {
        ...
        this.storage.getItem(watchFullUrl).pipe(
            map(item => {
                if (item != null) {
                    let watch: Watch = new Watch();
                    watch.parse(item);
            ...
            .subscribe((watchItem ) => {
                    if ( watchItem != null ) {
                        ...
                        this.storage.getItem(poolRefreshUrl).pipe(map(item => {
                            if (item != null) {
                                let watchOut: WatchOut = new WatchOut();
                                watchOut.parse(item);
                                return watchOut;
                            }
                        }))
                            ...
                            .subscribe((watchOut ) => {
                                if ( watchOut != null ) {
                                    ...
                                    for ( let value of valueList.childrens ) {
                                        ...
                                        this.storage.getItem(value.getUrl(this.rootUrl)).pipe(map(item => {
                                            if (item != null) {
                                                let obj: Obj = Obj.obixParse(item);
                                                return obj;
                                            }
                                        }))
    ...
    refreshPoolRefresh(poolRefreshUrl ) {
        // do the delete on backend server
        this.http.post(poolRefreshUrl, null).pipe(map(response => {
            let watchOut: WatchOut = new WatchOut();
            watchOut.parse(response);
            return watchOut;
        }))
            .subscribe((watchOut ) => {
        ...
        this.storage.getItem(watchFullUrl).pipe(
            ...
            .subscribe((watchItem ) => {
                    if ( watchItem != null ) {
                        ...
                        this.http.post(poolChangesUrl, null).pipe(map(response => {
                            let watchOut: WatchOut = new WatchOut();
                            watchOut.parse(response);
                            return watchOut;
                        }))
        ...
        this.storage.getItem(watchFullUrl).pipe(
            ...
            .subscribe((watchItem ) => {
                    if ( watchItem != null ) {
                        ...
                        this.http.post(addUrl, watchIn).pipe(map(response => {
                            let watchOutAdd: WatchOut = new WatchOut();
                            watchOutAdd.parse(response);
                            return watchOutAdd;
                        }))
                            .subscribe((watchOutAdd ) => {
                                this.storage.getItem(poolRefreshUrl).pipe(map(item => {
                                    if (item != null) {
                                        let watchOutStorage: WatchOut = new WatchOut();
                                        watchOutStorage.parse(item);
                                        return watchOutStorage;
                                    }
                                }))
    ...
    updateWatchItem(obj: Obj){
        ...
        this.http.get(objUrl).pipe(map(response => {
