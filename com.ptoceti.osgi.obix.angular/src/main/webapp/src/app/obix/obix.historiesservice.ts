...
export class HistoriesService {
    ...
    constructor(http: HttpClient, storage: LocalStorage, searchService: SearchService) {
        ...
        this.cache = new ReplaySubject<HistoryService>(1);
    ...
    initialize(rootUrl : string, serviceUrl : string) {
        ...
        this.storage.getItem(serviceUrl).subscribe((response) => {
            if (response) {
                let historyservice: HistoryService = new HistoryService;
                historyservice.parse(response);
                this.cache.next(historyservice);
            } else{
                // not available, get it from backend server
                this.http.get(serviceUrl).pipe(map(this.handleHistoryServiceResponse)).subscribe(historyservice => {
    ...
    getHistoriesList() {
        ...
        this.storage.getItem(HistoriesService.historiesListKey).pipe(
            map(item => {
                if (item != null) {
            ...
            .subscribe((list ) => {
                if ( list != null ) {
                    ...
                    observableConcat<History>(...observables).subscribe((watch) => {
                        this.historyStream.next(new HistoryAction(Action.Add, watch))
                    });
                    ...
                    this.searchService.search(searchRef).pipe(
                        ...
                        .subscribe((searchOut ) => {
                            ...
                            this.storage.setItem( HistoriesService.historiesListKey, searchList ).subscribe(() => {
                                ...
                                observableConcat<History>(...observables).subscribe((history) => {
                                    this.historyStream.next(new HistoryAction(Action.Add, history))
                                });
                            }, () => { } );
                        } )
    ...
    deleteHistoy(historyRef : Ref ) : Observable<HistoryAction> {
        ...
        this.storage.getItem(historyUrl).pipe(
            map(item => {
                if ( item != null ) {
            ...
            .subscribe((historyItem ) => {
                if ( historyItem != null ) {
                    ...
                    this.http.delete(historyUrl).pipe(map(response => {
                        let nil: Nil = new Nil();
                        nil.parse(response);
                        return nil;
                    }))
    ...
    getHistory(historyUrl ): Observable<History> {
        ...
        this.storage.getItem(<string> historyUrl, {schema: {type: 'string'}}).pipe(
            map(item => {
                if (item != null) {
                    let historyItem: History = new History();
                    historyItem.parse(item);
                    return historyItem;
                }
            }))
            .subscribe((historyItem ) => {
    ...
    refreshHistory(historyUrl: string, observer: Observer<History> ) {
        this.http.get(historyUrl).pipe(map(response => {
            let historyItem: History = new History();
    ...
    refreshHistoryRollup( historyRef : Ref, startRollup: Date, endRollup: Date){
        ...
        this.getHistoryByRef(historyRef).subscribe((history)=>{
            ...
            this.storage.getItem(rollupName).pipe(
                map(item => {
                    if (item != null) {
                        let rollup: HistoryRollupOut = new HistoryRollupOut();
                        rollup.parse(item);
                        return rollup;
                    }
                }))
                .subscribe((rollupOut) =>{
                    ...
                    if( rollupOut !== undefined){
                        ...
                        this.http.post(rollupOp.getUrl(this.rootUrl), rollupIn).pipe(
                            map(response => {
                                let roolupOutFromPost: HistoryRollupOut = new HistoryRollupOut();
                                roolupOutFromPost.parse(response);
                                return roolupOutFromPost;
                            }))
                ...
                .subscribe((rollupOut) =>{
                    ...
                    if( rollupOut !== undefined){
                        ...
                        if(historyRollupRecords.length == 0){
                            ...
                            this.http.post(rollupOp.getUrl(this.rootUrl), rollupIn).pipe(
                                map(response => {
                                    let roolupResult: HistoryRollupOut = new HistoryRollupOut();
                                    roolupResult.parse(response);
                                    return roolupResult;
                                }))
    ...
    getLast24History (historyRef : Ref ) : Observable<HistoryRollupOut> {
        ...
        this.getHistoryByRef(historyRef).subscribe((history)=>{
            ...
            this.storage.getItem(rollupOp.getUrl(this.rootUrl) + '/24/').pipe(
                map(item => {
                    if (item != null) {
                        let rollup: HistoryRollupOut = new HistoryRollupOut();
                        rollup.parse(item);
