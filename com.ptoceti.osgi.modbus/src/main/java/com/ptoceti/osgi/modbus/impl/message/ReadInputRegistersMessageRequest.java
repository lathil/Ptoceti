 ...
 * PROJECT : Modbus
 * FILENAME : ReadInputRegistersMessageRequest.java
 * 
...
public class ReadInputRegistersMessageRequest extends ModbusMessageRequest {

	private int firstRegister; // adress of starting input - 2 bytes
	private int nbRegisters; // number of input - 2 bytes
	
	public ReadInputRegistersMessageRequest(byte unitID ) {
		super();
		setFunctionID(ModbusMessage.READ_INPUT_REGISTERS );
		setUnitID(unitID);
	}
	
	public ReadInputRegistersMessageRequest(byte unitID, int register, int number ) {
		super();
		setFunctionID(ModbusMessage.READ_INPUT_REGISTERS);
		setUnitID(unitID);
